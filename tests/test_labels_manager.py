...
def test_new_label_creates_change(repo: MagicMock) -> None:
    ...
    desired = SharedConfig(
        labels=[Label(name="bug", color="d73a49", description="Something is broken")],
    )
...
def test_new_label_without_description(repo: MagicMock) -> None:
    ...
    desired = SharedConfig(labels=[Label(name="bug", color="d73a49")])
    repo.get_labels.return_value = []
...
def test_existing_label_updates_change(repo: MagicMock) -> None:
    ...
    desired = SharedConfig(labels=[Label(name="bug", color="d73a49", description="new")])
...
def test_existing_label_matches_exactly(repo: MagicMock) -> None:
    ...
    desired = SharedConfig(labels=[Label(name="bug", color="d73a49", description="same")])
    repo.get_labels.return_value = [existing]

    assert LabelsManager().plan(repo, desired) == []


def test_extra_label_is_deleted(repo: MagicMock) -> None:
    """A declared labels section is authoritative: a label absent from it is deleted."""
    ...
    # One CREATE for "bug" and one DELETE for the unlisted "stale".
    actions = {change.target: change.action for change in changes}
    assert actions == {"label:bug": Action.CREATE, "label:stale": Action.DELETE}
...
def test_color_normalized_no_phantom_diff(repo: MagicMock) -> None:
    ...
    desired = SharedConfig(labels=[Label(name="bug", color="#FF0000")])
    repo.get_labels.return_value = [existing]
